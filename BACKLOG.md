...
bricktron is **Linux-only** (C++11 + Allegro 5, CMake). There is no Windows build.
...
| T3 | **Catch / sticky paddle power-up** — classic Bricktron "Catch": a power-up that makes the ball stick to the paddle on contact; the player releases it with a key (SPACE) or after a timeout. Handle reset/serve interaction and the multi-ball case. | 🔬 |
